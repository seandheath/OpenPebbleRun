 ...
 *
 * Watch → Companion:
 *   2   CMD_STOP           uint8  (no payload — value ignored)
 *
 *   Key 1 (CMD_START) was retired with the pre-run screen; runs are now
 *   initiated from the companion's Home screen (spec §5.2.2). Number 1 is
 *   pinned — do not reuse.
 *
 * Companion → Watch:
 ...
 *
 *   Key 111 (RUN_FAILED) was retired with the pre-run start-error path;
 *   number 111 is pinned — do not reuse.
 *
 ...
 * messages (RUN_STARTED / metric updates). Registered via
...
/*
 * Send a payloadless uint8 command (currently only CMD_STOP). Returns true if
 * the message was queued. AppMessage delivers one at a time and ACKs each
 * (spec §4.5); callers should rate-limit to one in-flight send. Generic over
 * the key for future watch→companion commands.
 */
